...
class MockApiServer:
    def __init__(self, aiohttp_client):
        ...
        self.app.router.add_get(
            "/api/v1/inverter/grid/1029384756/realtime", self.get_inverter_realtime_grid
        )
        self.app.router.add_get(
            "/api/v1/inverter/battery/1029384756/realtime",
            self.get_inverter_realtime_battery,
        )
        self.app.router.add_get(
            "/api/v1/inverter/1029384756/realtime/input",
            self.get_inverter_realtime_input,
        )
        self.app.router.add_get(
            "/api/v1/inverter/1029384756/realtime/output",
            self.get_inverter_realtime_output,
        )

    async def client(self, username="myuser"):
        client = await self.aiohttp_client(self.app)
        return await SunsynkClient.create(
            username, "letmein", base_url=f"http://{client.host}:{client.port}"
        )
                ...
                "infos": [
                    {
                        ...
                        "gatewayVO": {"gsn": "E0192837465", "status": 2},
                        "sunsynkEquip": True,
                ...
                "infos": [
                    {
                        ...
                        "masterId": 54321,
                        "share": False,
                        "plantPermission": ["station.share.cancle"],
                        "existCamera": False,
                    }
                ],
            },
    ...
    async def get_inverter_realtime_output(self, request):
        payload = {
            ...
            "msg": "Success",
            "data": {
                "vip": [{"volt": "230.8", "current": "0.3", "power": -50}],
                "pInv": 9,
                "pac": -50,
                "fac": 50.0,
            },
